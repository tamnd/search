...
description: "Build a query tree, parse the compact query string or the JSON DSL, apply boosts, and run sx query."
weight: 20
---
...
`Search` resolves each hit's stored body. For sort, facets, or collapse instead of pure score ranking, use the request API in [facets and sorting](/guides/facets-and-sorting/).
...
For sort, facet, and collapse flags, see [facets and sorting](/guides/facets-and-sorting/). For vector and hybrid queries, see [vector search](/guides/vector-search/) and [hybrid search](/guides/hybrid-search/). The complete flag list is in the [CLI reference](/reference/cli/).
